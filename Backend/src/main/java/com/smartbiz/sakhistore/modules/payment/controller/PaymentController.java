...
public class PaymentController {
    ...
    public ResponseEntity<?> razorpayCallback(@RequestBody Map<String, Object> request) {
        try {
            ...
            // Try to find existing payment by razorpayOrderId first (payment is created with this when Razorpay order is created)
            Payment existingPayment = paymentRepository.findByRazorpayOrderId(razorpayOrderId);
            
            if (existingPayment != null) {
                // Payment exists - update it with payment details and mark as PAID
                ...
                Orders order = existingPayment.getOrders();
                if (order != null) {
                    order.setPaymentStatus(PaymentStatus.PAID);
                    ordersRepository.save(order);
                    System.out.println("✅ Updated order #" + order.getOrdersId() + " payment status to PAID");
                } else {
                    System.out.println("⚠️ Payment found but has no associated order");
                }
            } else {
                ...
                if (existingPayment != null) {
                    ...
                    if (order != null) {
                        ...
                        paymentService.markPaymentPaidWithRazorpayDetails(razorpayPaymentId, razorpayOrderId, razorpaySignature);
                        System.out.println("✅ Created new payment and marked as PAID for order #" + orderId);
                    } else {
    ...
    public ResponseEntity<?> syncPaymentStatus() {
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(500).body(Map.of("error", "Failed to sync payment status: " + e.getMessage()));
        }
    }
